...
The default global profile is `core`, which includes `propose`, `explore`, `apply`, `update`, `split`, `sync`, and `archive`. You can enable the expanded workflow commands with `openspec config profile` and then `openspec update`.
