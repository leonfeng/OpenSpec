...
/**
 * The split workflow instructions, authored once and rendered by both the
 * skill and command surfaces. Transformers rewrite canonical `/opsx:<id>`
 * tokens per surface downstream (see command-references.ts).
 */
export function getSplitChangeInstructions(): string {
  return `Split one OpenSpec change into multiple independently applyable changes.
...
export function getSplitChangeSkillTemplate(): SkillTemplate {
  return {
    ...
    instructions: getSplitChangeInstructions(),
    license: 'MIT',
...
export function getOpsxSplitCommandTemplate(): CommandTemplate {
  return {
    name: 'OPSX: Split',
    description: 'Split a change into independently applyable changes (Experimental)',
    category: 'Workflow',
    tags: ['workflow', 'artifacts', 'experimental'],
    content: getSplitChangeInstructions(),
  };
}
